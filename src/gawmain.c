...
void aw_create_main_window ( UserData *ud )
{
   ...
   /* create signal list box inside the left pane */
   ud->siglist_box = gtk_box_new (GTK_ORIENTATION_VERTICAL, 0);
...
void aw_rc_parse_gtkrc(UserData *ud)
{
   ...
   if (file_exists (rc)) {
      gtk_css_provider_load_from_path (provider, rc, NULL );
      ret++;
   }
   ...
   /* 3. Try original gaw directory for backward compatibility */
   filename = app_strcatdup ( "/usr/local/share/gaw", "/", rc, NULL);
   if (file_exists (filename)) {
      gtk_css_provider_load_from_path (provider, filename, NULL );
      ret++;
   }
   ...
   /* 5. Try original .gaw directory for backward compatibility */
   filename = app_strcatdup ( fsu_home_dir(), "/.gaw/", rc, NULL);
   if (file_exists (filename)) {
      gtk_css_provider_load_from_path (provider, filename, NULL );
      ret++;
   }
   ...
   if ( ! ret ) {
      msg_warning(_("file '%s' do not exist in standard paths."), rc );
   }
