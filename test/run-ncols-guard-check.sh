#!/bin/sh
# Acceptance node for the ncols == 2 guard hardening (checks N1..N3).
#
...
# test_ncols_guard_harness.c against it and runs the checks:
#
...
#   N3  an ncols=2 source still derives normally (regression lock).
#
# The harness prints a PASS marker and exits 0 only when all checks pass.
#
# Usage: test/run-ncols-guard-check.sh [SRCDIR]   (SRCDIR defaults to repo root)
set -u
...
WORKDIR=$(mktemp -d /tmp/gawm-ncolsguard.XXXXXX) || {
...
HARNESS=$SCRIPT_DIR/test_ncols_guard_harness.c
...
echo
echo "=== ncols-guard checks (N1..N3) ==="
OUT=$("$BIN" 2>&1)
STATUS=$?
printf '%s\n' "$OUT"

if printf '%s\n' "$OUT" | grep -q "^PASS: ncols-guard N1-N3 all satisfied"; then
    echo
    echo "VERDICT: GREEN - ncols-guard N1..N3 all pass"
    exit 0
fi

echo
echo "VERDICT: RED - at least one ncols-guard check failed"
exit 1
