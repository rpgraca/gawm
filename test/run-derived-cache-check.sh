#!/bin/sh
# Acceptance node for the derived-value min/max/range cache (checks G1..G3).
#
...
# links, runs, and exits 0 only when G1..G3 all pass.
#
...
echo
echo "=== derived-cache behaviour pass (G1..G3) ==="
OUT=$("$BIN" 2>&1)
STATUS=$?
printf '%s\n' "$OUT"

if [ "$STATUS" -eq 0 ] && printf '%s\n' "$OUT" | grep -q \
        "^PASS: derived-cache G1-G3 all satisfied"; then
    echo
    echo "VERDICT: GREEN - G1..G3 all pass"
    exit 0
fi
