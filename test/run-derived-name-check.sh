#!/bin/sh
# Regression: gawio derived-variable NAME resolution (Unit B).
#
# Builds a disposable out-of-tree gawm, then compiles and links the C harness
# `test_derived_name_resolv_harness.c` against the freshly built objects and
# runs it.  The harness builds an in-memory WDataSet with a two-column complex
# variable, derives magnitude/phase WaveVars, and feeds `coldatas V(out):mag`,
# `coldatas V(out):phase`, `coldatas V(out)`, and a bad name through the real
# aio_process_line path.  aio_coldatas_add resolves its argument through
# dataset_get_var_for_name; the derived vars live in wds->dvars.
#
# GREEN: exits 0 and prints "PASS: gawio derived-name resolution B1..B5".
#   This is an acceptance node; derived-name resolution was completed by the
#   magnitude/phase commit, so no production change is required.
set -u

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd -P) || exit 1

# Optional SRCDIR argument allows running against an explicit snapshot.
if [ "$#" -gt 1 ]; then
...
WORKDIR=$(mktemp -d /tmp/gawm-deriv.XXXXXX) || {
...
HARNESS=$SCRIPT_DIR/test_derived_name_resolv_harness.c
HARNESS_OBJ=$WORKDIR/test_derived_name_resolv_harness.o
HARNESS_BIN=$WORKDIR/test_derived_name_resolv
...
# Run the harness.  It must exit 0 and print PASS; a crash yields non-zero.
OUT=$("$HARNESS_BIN" 2>&1)
STATUS=$?
echo "$OUT"
if [ "$STATUS" -ne 0 ]; then
    echo "[FAIL] derived-name harness did not pass ($STATUS)" >&2
    exit 1
fi
if ! printf '%s\n' "$OUT" | grep -q "PASS: gawio derived-name resolution B1..B5"; then
    echo "[FAIL] missing PASS marker" >&2
    exit 1
fi
echo "[ok] harness run (gawio derived-name resolution ok)"
exit 0
